...
from service.ragfusion.utils.verify.version import satisfies_min_unstructured_version
...
class UnstructuredFileLoader(UnstructuredBaseLoader):
    ...
    def __init__(
        ...
        **unstructured_kwargs: Any,
        ...
        """Initialize with file path."""
        # Attention: If `file_or_buffer` are type of `BytesIO`,
        # you have to implement `_get_elements` in your sub-class, like
        #######################################
    ...
    def _get_elements(self) -> List:
        ...
        if isinstance(self.file_or_buffer, list):
            elements = []
            for file in self.file_or_buffer:
                if isinstance(file, Path):
                    file = str(file)
                elements.extend(partition(filename=file, **self.unstructured_kwargs))
            return elements
        else:
            if isinstance(self.file_or_buffer, Path):
                self.file_or_buffer = str(self.file_or_buffer)
            return partition(filename=self.file_or_buffer, **self.unstructured_kwargs)

    def _get_metadata(self) -> dict:
        return {
            "source": self.file_or_buffer,
